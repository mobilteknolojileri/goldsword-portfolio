...
const FeaturedApps = () => {
    ...
    const featuredProjects = PROJECTS.filter(p =>
        p.slug === 'qusafe-mobile' || p.slug === 'legends-of-aestera'
    )

    return (
        <section id="featured-apps" className="py-20 bg-gray-50 dark:bg-dark-900 overflow-hidden">
            <div className="section-container section-padding">
                <div className="text-center mb-16">
                    <h2
                        ref={titleRef}
                        className={`text-4xl md:text-5xl lg:text-6xl font-bold text-heading tracking-tight transition-all duration-700 ${titleVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'
                            }`}
                    >
                        Kendi Projelerim
                    </h2>
                    <p className={`mt-4 text-lg text-body transition-all duration-700 delay-100 ${titleVisible ? 'opacity-100' : 'opacity-0'
                        }`}>
                        ...
                        >
                            <Link href={project.customLink || `/portfolio/${project.slug}`}>
                                <div className="flex flex-col h-full">
                                    <div className={`h-64 bg-gradient-to-br ${project.color} relative overflow-hidden flex items-center justify-center p-8`}>
